...
const values = [
  [
    ...
    "Presenza quando serve, liberta quando il soggiorno chiede silenzio.",
  ],
  [
    "Case con carattere",
    "Ogni dimora racconta materiali, luce, paesaggio e ritmo del luogo.",
  ],
  [
    "Cura operativa",
    "Informazioni chiare, calendario predisposto e richieste gestite con precisione.",
  ],
];
      ...
      <section className="grid h-svh items-center bg-paper px-6 pb-16 pt-24 md:grid-cols-[0.82fr_1.18fr] md:px-22 md:pb-24 md:pt-32 fxl:px-35">
        <Reveal>
          ...
          <p className="mt-6 max-w-[620px] text-base leading-7 text-muted">
            {siteConfig.name} e un template per raccontare strutture ricettive
            con tono boutique: poche parole, immagini importanti e un percorso
            di prenotazione chiaro.
          </p>
        </Reveal>
        <SoftScale className="relative mt-10 min-h-[360px] overflow-hidden md:mt-0 md:min-h-[560px]">
          <Image
            src="/images/stone-bath.png"
            alt={`Dettagli di ospitalita e materiali naturali di ${siteConfig.name}`}
            fill
            priority
      ...
      <section className="grid items-center gap-10 bg-ivory px-6 py-20 md:grid-cols-[1fr_1fr] md:px-[88px] md:py-28 fxl:px-[140px] fxl:py-36">
        ...
        <Reveal>
          ...
          <h2 className="font-serif text-[clamp(2.4rem,4vw,4.8rem)] leading-[1]">
            Non vendere una notte. Raccontare un modo di svegliarsi.
          </h2>
          <p className="mt-6 text-base leading-7 text-muted">
            Il sito e pensato per strutture che vogliono comunicare atmosfera
            prima ancora dei servizi: una casa vacanza a {siteConfig.locality},
            una villa in affitto o un appartamento turistico devono sembrare
            luoghi da abitare, non prodotti in catalogo.
          </p>
        </Reveal>
      </section>
      ...
      <section className="bg-paper px-6 py-20 md:px-[88px] md:py-28 fxl:px-[140px] fxl:py-36">
        <StaggerReveal className="grid gap-8 md:grid-cols-3">
          {values.map(([title, text]) => (
            <StaggerItem key={title} className="pt-6 border-t border-ink/15">
